...
- **No ready sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check — STABLE ADVANCE**: GitHub stable moved v0.21.4 → **v0.21.5 (tag `v2026.9.24`, published 2026-09-24T10:09:38Z, prerelease=false)** — the "Desktop Plugin SDK Patch Release". Window since v0.21.4 (3 days): 1,610 non-merge commits, 4,828 changed files (+164,132/−149,440), 460 merged PRs, 475 closed issues (figures from the release body's own measurement commit `f97608f1`; the GitHub compare API reports `files: 300` — endpoint truncation cap, not the real count). Headlines: Desktop plugin SDK wave (composer draft API, session-list/row-decoration slots, sidebar nav prefs, model-pill label providers, typed settings/skills/toolsets/profiles bridges, sandboxed embed primitive, appearance-settings slot, public event bridge); Connectors page replacing the MCP tab with "Connect now" for freshly installed plugins' MCP servers; tools/skills live in every open chat without restart; Simple/Advanced interface mode; FR/DE/ES catalogs + RTL/LTR setting; custom model entry; function-key + dictation voice shortcuts; per-profile stop/start/restart with `gateway.standalone` opt-out; live dock showing standing `/goal` + queued prompts; kanban design pass; webhook deliveries mirrored into target chat; Bot Screen on hosted `-desktop` images; GPT-6 Sol/Terra/Luna + Claude Opus 5.5 in catalogs; Blender Lab + NVIDIA app/Broadcast plugins; hot-path performance work (config loading, tool registry, gateway message handling, model picker). Curated notes deferred to v0.22.0. Verified against the paginated releases list. PyPI still lags at 0.19.0. GitHub repo security advisories: none published (0).
...
- **Added**: `hermes-0215-desktop-plugin-sdk-patch-release` — new reference doc for the v0.21.5 patch release (upstream-sourced ingestion pattern, same as 0212/0213/0214); `docs/index.yaml` regenerated with doc_count 30, entry inserted adjacent to the other release references, no reordering churn. Index validated programmatically: 30 entries, zero duplicate slugs, zero missing paths, doc_count field consistent.
- **Updated**: `hermes-0214-fleet-rollup-patch-release` — cross-link to the new 0.21.5 doc (successor patch) in Related docs.
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 11,610 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 9,347 pre-fetch (yesterday's cached number); post-fetch rev-list 11,610 — largest 24h delta of the series, upstream burst ~2,263/24h post-v0.21.5-tag; was 9,347 on 09-24). Behind-tag now 9,509 for `v2026.9.24` / v0.21.5, tag-to-main 2,101. Identity check passed (9,509+2,101=11,610). v0.21.5 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: upstream-sourced ingestion run — robust session discovery (`grep -E 'status:\s*["']?ready'` over all 8 session files, statuses enumerated individually), release verified against the paginated releases list, zero repo advisories, index re-validated (YAML parse + slug/path/frontmatter checks via script file), gitleaks passed before commit, staged set limited to the new doc + 0214 update + index + changelog + logs. One mid-run correction worth noting: a sed line-insert landed the new index entry one line too low, orphaning the 0214 block's `hermes_version_min` key; detected via duplicate/grep checks and fixed by re-inserting the key in its block — final index validated clean (30/30/0 dupes).
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.24, https://github.com/NousResearch/hermes-agent/compare/v2026.9.21...v2026.9.24, https://pypi.org/pypi/hermes-agent/json
...
- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.4 (tag `v2026.9.21`, published 2026-09-21T18:10:55Z, ingested 2026-09-22 as `hermes-0214-fleet-rollup-patch-release`); verified against the paginated releases list. No newer tags or prereleases beyond v2026.9.21. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 8,359 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 8,359 too — zero drift; was 7,977 yesterday — upstream burst ~382/24h, all post-tag activity: behind-tag count unchanged at 7,871 for `v2026.9.21` / v0.21.4, tag-to-main now 488). Range-identity sanity check passed: behind-tag (7,871) + tag-to-main (488) = behind-main (8,359). v0.21.4 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: clean skip run — robust session discovery (`grep -n '^status:'` over all 8 session files, statuses enumerated individually), release verified against the paginated releases list, zero repo advisories, zero docs/index churn, gitleaks passed before commit, only `logs/daily-elaboration.log` + this changelog block staged.
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.21, https://pypi.org/pypi/hermes-agent/json
...
- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.3 (tag `v2026.9.14`, published 2026-09-14T16:04:14Z, ingested 2026-09-16 as `hermes-0213-remote-gateway-patch-release`); verified against the paginated releases list. No newer tags or prereleases beyond v2026.9.14. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 7,646 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 7,646 too — zero drift; was 6,549 yesterday — upstream burst ~1,097/24h, all post-tag activity: behind-tag count unchanged at 2,698 for `v2026.9.14` / v0.21.3, tag-to-main now 4,948). Range-identity sanity check passed: behind-tag (2,698) + tag-to-main (4,948) = behind-main (7,646). v0.21.3 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
...
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.14, https://pypi.org/pypi/hermes-agent/json

## 2026-09-20

- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.3 (tag `v2026.9.14`, published 2026-09-14T16:04:14Z, ingested 2026-09-16 as `hermes-0213-remote-gateway-patch-release`); verified against the paginated releases list. No newer tags or prereleases beyond v2026.9.14. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 6,549 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 6,536 pre-fetch; pre-fetch <= post-fetch, consistent drift from concurrent upstream pushes; was 5,621 yesterday — upstream burst ~928/24h, all post-tag activity: behind-tag count unchanged at 2,698 for `v2026.9.14` / v0.21.3, tag-to-main now 3,851). Range-identity sanity check passed: behind-tag (2,698) + tag-to-main (3,851) = behind-main (6,549). v0.21.3 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: clean skip run — robust session discovery (`grep -E 'status:\s*["']?ready'` over all 8 session files, statuses enumerated individually), zero docs/index churn, gitleaks passed before commit, only `logs/daily-elaboration.log` + this changelog block staged.
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.14, https://pypi.org/pypi/hermes-agent/json
...
- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.3 (tag `v2026.9.14`, published 2026-09-14T16:04:14Z, ingested 2026-09-16 as `hermes-0213-remote-gateway-patch-release`); verified against the paginated releases list. No newer tags or prereleases beyond v2026.9.14. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 5,621 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 5,621 too — zero drift; was 4,705 yesterday — upstream burst ~916/24h, all post-tag activity: behind-tag count unchanged at 2,698 for `v2026.9.14` / v0.21.3, tag-to-main now 2,923). Range-identity sanity check passed: behind-tag (2,698) + tag-to-main (2,923) = behind-main (5,621). v0.21.3 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: clean skip run — robust session discovery (`grep -E 'status:\s*["']?ready'` over all 8 session files, statuses enumerated individually), zero docs/index churn, gitleaks passed before commit, only `logs/daily-elaboration.log` + this changelog block staged.
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.14, https://pypi.org/pypi/hermes-agent/json
...
- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.3 (tag `v2026.9.14`, published 2026-09-14T16:04:14Z, ingested 2026-09-16 as `hermes-0213-remote-gateway-patch-release`); verified against the paginated releases list. No newer tags or prereleases beyond v2026.9.14. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 4,705 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 4,705 too — zero drift; was 4,396 yesterday — upstream burst ~309/24h, all post-tag activity: behind-tag count unchanged at 2,698 for `v2026.9.14` / v0.21.3, tag-to-main now 2,007). Range-identity sanity check passed: behind-tag (2,698) + tag-to-main (2,007) = behind-main (4,705). v0.21.3 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: clean skip run — robust session discovery (`grep -E 'status:\s*["']?ready'` over all 8 session files, statuses enumerated individually), zero docs/index churn, gitleaks passed before commit, only `logs/daily-elaboration.log` + this changelog block staged.
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.14, https://pypi.org/pypi/hermes-agent/json
...
- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.3 (tag `v2026.9.14`, published 2026-09-14T16:04:14Z, ingested 2026-09-16 as `hermes-0213-remote-gateway-patch-release`); verified against the paginated releases list. No newer tags or prereleases beyond v2026.9.14. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 4,396 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 4,351 pre-fetch; pre-fetch <= post-fetch, consistent drift from concurrent upstream pushes; was 3,769 yesterday — upstream burst ~627/24h, all post-tag activity: behind-tag counts unchanged at 2,698 for `v2026.9.14` / v0.21.3 and 1,659 for `v2026.9.11` / v0.21.2). Range-identity sanity check passed: behind-tag (2,698) + tag-to-main (1,698) = behind-main (4,396). v0.21.3 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: clean skip run — robust session discovery (`grep -E 'status:\s*["']?ready'` over all 8 session files, statuses enumerated individually), zero docs/index churn, gitleaks passed before commit, only `logs/daily-elaboration.log` + this changelog block staged.
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.14, https://pypi.org/pypi/hermes-agent/json
...
- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.2 (tag `v2026.9.11`, published 2026-09-11T19:20:31Z, ingested 2026-09-12 as `hermes-0212-state-db-patch-release`); verified against the paginated releases list. No new prereleases/tags beyond v2026.9.11. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 2,573 commits behind upstream main per post-fetch rev-list (`hermes --version` printed 2,466 pre-fetch; pre-fetch <= post-fetch, consistent drift from concurrent upstream pushes; was 2,101 yesterday — upstream burst ~472/24h, all post-tag activity: behind-tag counts unchanged at 673 for `v2026.9.7` / v0.21.1 and 1,659 for `v2026.9.11` / v0.21.2). v0.21.2 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: clean skip run — robust session discovery (grep -E over status field, quoted/unquoted, all 8 session files enumerated individually), zero docs/index churn, gitleaks passed before commit, only `logs/daily-elaboration.log` + this changelog block staged.
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.11, https://pypi.org/pypi/hermes-agent/json

## 2026-09-13

- **No sessions to process** — zero sessions with status: ready (all 8 sessions status: processed; last session 2026-06-09).
- **Upstream check**: GitHub stable unchanged at v0.21.2 (tag `v2026.9.11`, published 2026-09-11T19:20:31Z, ingested 2026-09-12 as `hermes-0212-state-db-patch-release`); verified against the paginated releases list. No new prereleases/tags beyond v2026.9.11. PyPI still lags at 0.19.0. GitHub security advisories for the repo: none published. No docs or index changes (skip contract: zero churn).
- **Local runtime**: still v0.21.0 snapshot (+5,322 past tag `v2026.8.31`), now 2,101 commits behind upstream main per `hermes --version` (rev-list cross-check post-fetch: 2,101, zero drift; was 1,752 yesterday — upstream burst ~349/24h, all post-tag activity: behind-tag counts unchanged at 673 for `v2026.9.7` / v0.21.1 and 1,659 for `v2026.9.11` / v0.21.2). v0.21.2 upgrade available via `hermes update`; not applied by this docs run (docs cycle does not upgrade the runtime).
- **Self-assessment**: clean skip run — robust session discovery (`grep -E 'status:\s*["']?ready'` over all 8 session files, statuses enumerated individually), zero docs/index churn, gitleaks passed before commit, only `logs/daily-elaboration.log` + this changelog block staged.
- **Sources**: no ready session files; upstream check — https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.11, https://pypi.org/pypi/hermes-agent/json
